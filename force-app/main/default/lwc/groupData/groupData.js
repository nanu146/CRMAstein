...
export default class GroupData extends LightningElement {    
    @api title = "Group Text Data";
    @api results;
    @api model = "sfdc_ai__DefaultOpenAIGPT4_32k";
    @api setSelection;
    @api selectMode;
    
    prompt = "Summarize based on identifying common themes, topics and issues"
    ...
    error = undefined;
    spinner = false;
    // watch out: selection_changed uses tri-state logic with undefined, false, true
    // undefined is used for initalization, so that initial selections do not turn it
    // (in stateChangeCallback()) to true.
    selection_changed = undefined; 
    ...
    generatePrompt() {
        return `You are given Data as a JSON array.  Your task it to group the items in this JSON array 
based on a question or instruction.  In addition to textual explanation, return the JSON data in the 
following format.  Wrap the JSON data in markdown with "\`\`\`json" and "\`\`\`" as usual:

\`\`\`json
[
    {
        topic: "a short desciption of the data",
        description: "a longer description",
        data: [ /* array of the corresponding items in their original format */ ]
    }, 
    ...
]
\`\`\`

Your question or instuction:
${this.prompt}
...
`
    ...
    async run() {
        ...
        try {
            this.parseOuputAndSetSelection();            
        }
        catch(e) {
            console.log("Failed to parse output and set selection");
            console.log(e);            
        }
    ...
    parseOuputAndSetSelection() {
        const jsonRegex = /```json([\s\S]*?)```/g;
        console.log("Executing regex");
        console.log(this.output)
        let m = jsonRegex.exec(this.output);
        console.log("m = " + m);        
        if(m !== null) {
            ...
            console.log(JSON.stringify(json[0].data, null, 4));
            this.setSelection(json[0].data);
        }
        else {
